...
if weights_path is not None:
    model = PPO.load(weights_path, env = env, tensorboard_log = logdir, n_steps = TIMESTEPS)
else:
    # Create the model with PPO algorithm
    model = PPO("MultiInputPolicy", env, verbose = 1, tensorboard_log = logdir, n_steps = TIMESTEPS)

# Training parameters
SAVE_INTERVAL = 10000 # Number of iterations to save
TOTAL_TRAINING_TIMESTEPS = 1000000 # Total number of iterations for training
